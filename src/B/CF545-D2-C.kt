...
fun main() {
    ...
    dp=Array(arr.size){LongArray(2){-1} }
    println(dp(0,-1000,arr))
}

fun dp(ind:Int,flag:Int,arr:ArrayList<Pair<Long,Long>>):Long{
    if(ind==arr.size){
        return 0
    }
    if(ind==0){
        return 1+dp(ind+1,0,arr)
    }
    if(dp[ind][flag]!=-1L){
        return dp[ind][flag]
    }
    val previous:Long
    if(flag==0){
        previous=arr[ind-1].first
    }
    else{
        previous=arr[ind-1].first+arr[ind-1].second
    }
    if(previous>=arr[ind].first){
        return Int.MIN_VALUE.toLong()
    }
    if(arr[ind].first-arr[ind].second>previous){
        dp[ind][flag]= 1+dp(ind+1,0,arr)
        return dp[ind][flag]
    }
    else{
        dp[ind][flag]= maxOf(dp(ind+1,0,arr),1+dp(ind+1,1,arr))
        return dp[ind][flag]
    }
}
