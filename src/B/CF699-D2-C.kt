...
fun main() {
    ...
    for (i in 1 until n){
        when(arr[i]){
            1->count[i][1]= max(count[i-1][0]+1,count[i-1][2]+1)
            2->count[i][2]= max(count[i-1][0]+1,count[i-1][1]+1)
            3->{
                count[i][1]= max(count[i-1][0]+1,count[i-1][2]+1)
                count[i][2]= max(count[i-1][0]+1,count[i-1][1]+1)
            }
        }
        count[i][0]= max(count[i-1][0],max(count[i-1][1],count[i-1][2]))
    }
    println(n-max(count[n-1][0],max(count[n-1][1],count[n-1][2])))
}
