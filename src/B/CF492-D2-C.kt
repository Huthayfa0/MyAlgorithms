...
fun main() {
    val (n,r,avg)=readlnInts()
    var count=n.toLong()*avg
    var arr=Array(n){
        readIntArray().also { count-=it[0]
        it[0]=r-it[0]}.takeUnless { it[0]==0 }
    }.filterNotNull()
    arr=arr.sortedBy { it[1] }
    var sum=0L
    var i=0
    while (count>0){
        sum+= min(arr[i][0].toLong(),count)*arr[i][1]
        count-=arr[i][0]
        i++
    }
    println(sum)
