...
fun main() {
    val change= listOf(
        intArrayOf(0,1),intArrayOf(1,1),intArrayOf(1,0),intArrayOf(0,-1),intArrayOf(-1,-1),intArrayOf(-1,0),intArrayOf(-1,1),intArrayOf(1,-1)
    )
    var (x0,y0,x1,y1)=readlnInts()
    x0--
    y0--
    x1--
    y1--
    val n=readlnInt()
    val arr=HashSet<Pair<Int,Int>>()
    repeat(n){
        var (ri,ai,bi)=readlnInts()
        ri--
        for (i in (ai-1)..(bi-1))
            arr.add(Pair(ri,i))
    }
    var ans=-1
    val queue = LinkedList<Triple<Int,Int,Int>>() // list used as a queue
    val isVisited =HashSet<Pair<Int,Int>>()
    queue.offer(Triple(x0,y0,0)) // Enqueue v
    isVisited.add(Pair(x0,y0))// Mark it visited
    while (!queue.isEmpty()) {
        ...
        change.forEach{ e ->
            val newx=u.first+e[0]
            val newy=u.second+e[1]
            if (newx >= 0 && newx < 1000_000_000 && newy >= 0 && newy < 1000_000_000) {

                if (!isVisited.contains(Pair(newx,newy))&&arr.contains(Pair(newx,newy))) { // e.v is w in Listing 28.11
                    queue.offer(Triple(newx,newy,u.third+1)) // Enqueue w
                    isVisited.add(Pair(newx,newy)) // Mark w visited
                }
            }
        }
    }
