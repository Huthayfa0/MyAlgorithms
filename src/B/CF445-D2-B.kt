...
fun main() {
    val (n,m)=readlnInts()
    val adjacencyList= List<MutableList<Int>>(n){ArrayList()}
    repeat(m){
        val (x,y)=readlnInts().map { it-1 }
        adjacencyList[x].add(y)
        adjacencyList[y].add(x)
    }
    println(BigInteger.ONE.shiftLeft(dfs(n,adjacencyList)))
}
...
//Graphs
private fun dfs(size: Int, adjacencyList: List<List<Int>>, v: Int = 0):Int {
