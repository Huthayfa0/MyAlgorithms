...
fun main() {
    val n=readlnInt()
    val arr=Array(n){readlnInts()}.withIndex().sortedByDescending { it.value[1] }
    val m=readlnInt()
    val tables=readlnInts().withIndex().sortedBy { it.value }
    val taken=IntArray(m){-1}

    fun binary( start:Int=0, end:Int=m-1,can:(Int)->Boolean):Int{
        var s=start
        var e=end
        repeat(13){
            val mid = (s + e) / 2
    ...
    for (x in arr){
        if (tablesC==0)break
        var a=binary { it<m&&(tables[it].value>=x.value[0]) }
        if (a==-1)
            continue
        while (a<m&&taken[a]!=-1)
            a++
    ...
    println("${m-tablesC} $sum")
    for (i in taken.indices){
        if(taken[i]!=-1)
            println("${taken[i]+1} ${tables[i].index+1}")
    }
}
