...
fun main() {
    ...
    val (n,m)=readlnInts()
    val arr=Array(n){i->
        readln().withIndex().map { it.value != '0' }
    }

    val ans=Array(n){IntArray(m){Int.MAX_VALUE}}
    fun bfs(i:Int,j:Int,x:Int=0){
        if (i==n||i==-1||j==-1||j==m)
            return
        if (x>=ans[i][j])
            return
        ans[i][j]=x
        bfs(i-1,j,x+1);bfs(i+1,j,x+1);bfs(i,j+1,x+1);bfs(i,j-1,x+1)
    }
    arr.forEachIndexed { index, list -> list.forEachIndexed { indexa, b ->if (b)bfs(index,indexa)  } }
