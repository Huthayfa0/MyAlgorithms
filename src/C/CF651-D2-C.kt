...
fun main() {
    val n=readlnInt()
    val pairs=Array(n){readlnLongs()}
    var ans=0L
    val x=pairs.groupingBy { it[0] }.eachCount()
    val y=pairs.groupingBy { it[1] }.eachCount()
    for (xx in x){
        if (xx.value>1)
            ans+=(xx.value.toLong()-1)*xx.value/2
    }
    for (xx in y){
        if (xx.value>1)
            ans+=(xx.value.toLong()-1)*xx.value/2
    }
    ans-=pairs.groupingBy { it }.eachCount().map {xx-> if(xx.value==1)0 else (xx.value.toLong()-1)*xx.value/2  }.sum()
    /*for (i in 0 until (n-1)){
        for(j in (i+1) until n){
            val x= abs(pairs[i][0]-pairs[j][0])+ abs(pairs[i][1]-pairs[j][1])
            val y=(pairs[i][0]-pairs[j][0])*(pairs[i][0]-pairs[j][0])+ (pairs[i][1]-pairs[j][1])*(pairs[i][1]-pairs[j][1])
            if (x*x==y)
                ans++
        }
    }*/
