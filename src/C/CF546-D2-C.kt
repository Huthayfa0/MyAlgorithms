...
fun main() {
    val n=readlnInt()
    val arr=LinkedList(readlnInts().drop(1))
    val arr2=LinkedList(readlnInts().drop(1))
    var count=0
    val pairs=HashSet<Pair<String,String>>()
    while (arr.isNotEmpty()&&arr2.isNotEmpty()){
        count++
        if (!pairs.add(Pair(arr.toString(),arr2.toString()))){
            println(-1)
            return
        }
        val x=arr.pop()
    ...
    println("$count ${if (arr2.isEmpty()) 1 else 2}")
