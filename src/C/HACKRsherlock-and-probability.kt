...
val t=readlnInt()
    repeat(t){
        val (n,k)=readlnInts()
        val arr=readln().map { it.digitToInt() }
        var c=arr[0]
        for (i in 1..min(k,n-1))
            c += arr[i]
        var ans=if (arr[0]==1)c.toLong() else 0L

        for (i in 1 until n){
            if (i-k>0) c-=arr[i-k-1]
            if (i+k<n) c+=arr[i+k]
            if (arr[i]==1) ans+= c.toLong()
        }
        fun gcd(a:Long,b:Long):Long=if (b==0L)a else gcd(b,a%b)
        val g=gcd(n*n.toLong(),ans)
        println("${ans/g}/${n.toLong()*n/g}")
    }
}
