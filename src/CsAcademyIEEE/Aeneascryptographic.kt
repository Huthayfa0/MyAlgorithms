...
fun main() {
    val r=readlnInt()
    val v=DoubleArray(26){readlnStrings()[1].toDouble()}
    val arr=Array(26){i->
        DoubleArray(26){j-> sqrt(2-2* cos(Math.toRadians(v[i]-v[j])))}
    }
    var s=1.0
    val str=readln().toLowerCase().dropWhile { !it.isLetter() }
    var x=str[0]
